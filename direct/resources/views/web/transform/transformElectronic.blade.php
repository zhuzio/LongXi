...
@section('content')
    ...
    <div class="cpContainer">
        <p>增值积分转换</p>
        <div class="cpList"><span class="cpTitle">总增值积分</span><i>{{ $data['total'] }}</i></div>
        {{--<div class="cpList"><span class="cpTitle">最低转换金额</span><i>100</i></div>--}}
        <div class="cpList"><span class="cpTitle">转换</span><input type="number" placeholder="输入需转换金额" class="cNum"></div>
    ...
    <script>
        ...
        function cpBtn() {
            var _cNum = $(".cNum").val(),
                cpt = $('.z_img').val();
            if (_cNum == '') {
                layer.msg('转换金额不能为空', {icon: 5});
                return false;
            };
            if (_cNum%100 !== 0){
                layer.msg('转换金额必须为100的整倍数', {icon: 5});
                return false;
            };
            if (cpt == '') {
